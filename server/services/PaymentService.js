...
const syncEarningsStatus = async (providerId) => {
  try {
    ...
    if (provider) {
      ...
      provider.wallet.availableBalance = parseFloat(totalEarning.toFixed(2));
      provider.wallet.pendingWithdrawal = parseFloat(pendingWithdrawal.toFixed(2));
      provider.wallet.totalWithdrawn = parseFloat(totalWithdrawn.toFixed(2));
