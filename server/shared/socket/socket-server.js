...
const { addUser, removeUser } = require('./user-socket-map');
const { setIO: setNotifIO } = require('../../features/notification/notification-helper');
const Booking = require('../../features/booking/booking-model');
const Provider = require('../../features/provider/provider-model');
...
const initSocket = (httpServer) => {
    ...
    io.on('connection', (socket) => {
        ...
        (async () => {
            try {
                const ChatRoom = require('../../features/chat/chat-room-model');
        ...
        socket.on('provider-location-update', async (payload) => {
            try {
                ...
                const { latLngToS2CellId } = require('../utils/s2-helper');
                ...
                if (arrivalDist <= 100 && !booking.providerReached) {
                    ...
                    const { sendNotification } = require('../../features/notification/notification-helper');
                    try {
        ...
        socket.on('chat-send', async (payload) => {
            try {
                ...
                // Build message
                const { getSocketId } = require('./user-socket-map');
