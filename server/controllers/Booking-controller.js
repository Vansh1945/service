...
const getStartPin = (booking) => BookingService.getStartPin(booking);
const getCompletionPin = (booking) => BookingService.getCompletionPin(booking);
const ensureAndPersistPins = async (bookingId, bookingObj, session = null) => BookingService.ensureAndPersistPins(bookingId, bookingObj, session);
const getFailedAttempts = (booking) => BookingService.getFailedAttempts(booking);
const getLockoutTime = (booking) => BookingService.getLockoutTime(booking);
const recordPinFailure = async (booking, isStart, session = null) => BookingService.recordPinFailure(booking, isStart, session);
const resetPinFailures = async (booking, session = null) => BookingService.resetPinFailures(booking, session);
const getTargetLocation = (booking) => BookingService.getTargetLocation(booking);
const setTargetLocation = async (booking, latitude, longitude, session = null) => BookingService.setTargetLocation(booking, latitude, longitude, session);
const getBookingAddressLocation = (booking) => BookingService.getBookingAddressLocation(booking);
const calculateDistance = (lat1, lon1, lat2, lon2) => ProviderAssignmentService.calculateDistance(lat1, lon1, lat2, lon2);
const getFraudScore = (booking) => BookingService.getFraudScore(booking);
...
const sanitizeStatusHistoryForProvider = (statusHistory) => BookingService.sanitizeStatusHistoryForProvider(statusHistory);
