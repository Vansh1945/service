...
const { fetchImageBuffer, generateLetterheadDocument } = require('../booking/agreement-generator');
...
exports.previewTemplate = async (req, res, next) => {
  try {
    ...
    const { generateLetterheadDocument, stripHtml } = require('../booking/agreement-generator');
    ...
    const pdfBuffer = await generateLetterheadDocument(MOCK_PROVIDER, 'PREVIEW', async (doc, y, currentConfig, sigBuf, stampBuf) => {
      ...
      if (parsedSignatory || fields.hasCompanySeal) {
        ...
        const { fetchImageBuffer, drawAuthorizedSignOff } = require('../booking/agreement-generator');
