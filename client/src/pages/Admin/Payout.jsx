              ...
              {withdrawals.map((withdrawal) => (
                <tr key={withdrawal._id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    ...
                    <div className="mt-2">
                      <span className="text-xs px-2 py-1 bg-gray-100 text-gray-700 rounded font-poppins mr-2">
                        ID: {withdrawal.provider?._id?.substring(0, 8) || 'N/A'}
                      </span>
                      <span className="text-xs px-2 py-1 bg-primary/10 text-primary rounded font-poppins font-medium">
                        Wallet: ₹{(withdrawal.provider?.wallet?.availableBalance || 0).toLocaleString('en-IN')}
                      </span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      ...
                      {(withdrawal.status === 'requested' || withdrawal.status === 'processing' || withdrawal.status === 'under_review') && (
                        <>
                          <button
                  ...
                  {Array.from({ length: Math.ceil(total / limit) }, (_, i) => i + 1)
                    ...
                    .map(p => (
                      <button
                        key={p}
                        onClick={() => setPage(p)}
                        className={`w-9 h-9 flex items-center justify-center rounded text-sm ${p === page
            ...
            <div className="mb-6 p-4 bg-green-50 rounded border border-green-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-primary mt-1 font-poppins">Wallet Balance: ₹{(selectedWithdrawal.provider?.wallet?.availableBalance || 0).toLocaleString('en-IN')}</p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded p-5 border border-gray-200">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <p className="text-sm text-gray-600 font-poppins">Wallet Balance</p>
                      <p className="font-bold text-primary font-roboto">₹{(selectedDetails.provider?.wallet?.availableBalance || 0).toLocaleString('en-IN')}</p>
                    </div>
                  </div>
                </div>
