...
const AdminQuestions = () => {
  ...
  const handleBulkUpload = async () => {
    if (!bulkCategory) {
      showToast('Please select a category for bulk upload', 'error');
      return;
    }
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between">
            ...
            <div className="mt-4 md:mt-0 flex items-center gap-3">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors ${showStats
                ...
                {Object.entries(statistics.byCategory).map(([category, count]) => (
                  <div key={category} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${category === 'electrical' ? 'bg-primary' : 'bg-accent'
                        }`}></div>
            ...
            {showBulkUpload && (
              <div className="bg-white rounded-xl shadow-md p-5 border border-gray-100">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-secondary mb-2 font-medium">Category for Bulk Upload *</label>
                    <select
                      name="bulkCategory"
                      className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
                      value={bulkCategory}
