  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-inter text-slate-800">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Tab Navigation as Stats Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-slide-up">
          {[
            { id: 'ip', label: 'Same IP Detection', icon: FiMapPin, value: data.ip?.length || 0, subtext: 'IP correlation alerts', color: 'text-indigo-600', bg: 'bg-indigo-50' },
            { id: 'device', label: 'Device Abuse', icon: FiSmartphone, value: data.device?.length || 0, subtext: 'Device correlation alerts', color: 'text-amber-600', bg: 'bg-amber-50' },
        ...
        <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="text-sm font-semibold text-slate-600">
            Showing logs for <span className="text-indigo-600 font-bold capitalize">{activeTab === 'ip' ? 'Same IP' : activeTab === 'device' ? 'Device Abuse' : activeTab === 'cancellation' ? 'Cancellation' : 'Active Sessions'}</span>
          </div>
          {/* Quick Filters */}
