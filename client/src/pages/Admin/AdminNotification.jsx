...
const AdminNotification = () => {
    ...
    const applyPreset = (preset) => {
        ...
        setForm({
            ...
            body: preset.body,
            url: preset.url,
            targetZones: preset.targetZones || []
        });
        setStatus(null);
        setResult(null);
    };
    ...
    const handleZoneToggleCascade = (zone, isCreate) => {
        ...
        if (isCreate) {
            const currentSelected = form.targetZones || [];
            ...
            setForm(prev => ({ ...prev, targetZones: newZones }));
        } else {
            const currentSelected = (editModal.item.targetZones || []).map(z => z._id || z);
            let newZones = [...currentSelected];
            if (currentSelected.includes(zone._id)) {
                // DESELECT logic
                newZones = newZones.filter(id => id !== zone._id);
            ...
            setEditModal(prev => ({ ...prev, item: { ...prev.item, targetZones: newZones } }));
        }
    };
            ...
            >
                {editModal.item && (
                    <form onSubmit={handleUpdate} className="space-y-4">
                        ...
                        {/* Edit Target Zones Multi-Select */}
                        <div>
                            <HierarchicalZoneSelector
                                zones={zones}
