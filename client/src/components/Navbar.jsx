...
const ActionButton = ({ icon, text, variant = 'primary', fullWidth = false, path, onClick }) => {
  ...
  return (
    ...
    >
      ...
      {/* Subtle scale effect */}
      <div className={`absolute inset-0 rounded-lg border-2 border-transparent transition-all duration-300 transform scale-105 opacity-0 group-hover:opacity-100 ${variant === 'primary' ? 'border-white/20' : 'border-primary/10'
        }`} />
    </Link>
  );
};
